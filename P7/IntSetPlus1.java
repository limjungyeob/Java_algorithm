...
public class IntSetPlus1 {
    ...
    //집합 s에 복사합니다.
    public void copyTo(IntSetPlus1 s){
    ...
    //집합 s를 복사합니다.
    public void copyFrom(IntSetPlus1 s){
    ...
    //집합 s와 같은지 확인합니다.
    public boolean equalTo(IntSetPlus1 s){
    ...
    public void unionOf(IntSetPlus1 s1, IntSetPlus1 s2){
    ...
    //집합 s와의 교집합을 구합니다.
    public boolean retainUnion(IntSetPlus1 s){
        boolean flag = false;
        for(int i=0; i<num; i++){
    ...
    //집합s와의 차집합을 구합니다.
    public boolean removeUnion(IntSetPlus1 s){
        boolean flag = false;
        for(int i=0; i<num; i++){
    ...
    //집합 s의 부분집합인지 확인합니다.
    public boolean isSubsetOf(IntSetPlus1 s){
        for(int i=0; i<num; i++){
            int j=0;
            for( ; j<s.num;j++){
                if(set[i] == s.set[i]){
    ...
    //집합 s의 진부분집합인지 확인합니다.
    public boolean isProperSubsetOf(IntSetPlus1 s){
    ...
    //s1과 s2의 교집합을 복사합니다.
    public void intersectioOf(IntSetPlus1 s1, IntSetPlus1 s2){
        clear();
    ...
    //s1과 s2의 차집합을 복사합니다.
    public void diffenrenceOf(IntSetPlus1 s1, IntSetPlus1 s2){
        clear();
        for(int i=0; i<s1.num; i++){
            if(!s2.contains(s1.set[i])){
                add(s1.set[i]);
