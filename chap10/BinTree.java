...
public class BinTree<K,V> {
	...
	private void addNode(Node<K,V> node, K key, V data) {
		...
		if (cond == 0){                             // key가 이진검색트리에 이미 있음
			return;										
        }else if (cond < 0) {                       //작으면 왼쪽 서브트리
			if (node.left == null){                 //노드의 왼쪽 서브트리가 없다면 
				node.left = new Node<K,V>(key, data, null, null);       //new Node 호출
            }else{
				addNode(node.left, key, data);			// 왼쪽 서브 트리에 주목
            }
		} else {                                    //크면 오른쪽 서브트리
			if (node.right == null){                //노드의 오른쪽 서브트리가 없다면
				node.right = new Node<K,V>(key, data, null, null);      //new Node 호출
            }else{
